...
# We generate 40-dimensional fbanks on each frame; fbank.conf is overwritten here.
...
if [ ! -f  $working_dir/decode.done ]; then
  ...
  # Decode test; here "norm-vars" has to be consistent with steps_pdnn/build_nnet_pfile.sh
  steps_pdnn/decode_dnn.sh --nj 20 --beam 22 --latbeam 12 --scoring-opts "--min-lmwt 2 --max-lmwt 10" \
    --cmd "$decode_cmd" --norm-vars true \
