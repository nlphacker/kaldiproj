...
if [ ! -f $working_dir/cnn.fine.done ]; then
  ...
  $cmd $working_dir/log/cnn.fine.log \
    ...
    $pythonCMD pdnn/run_CNN.py --train-data "$working_dir/train.pfile,partition=1000m,random=true,stream=true" \
                          ...
                          --full-nnet-spec "1024:1024:$num_pdfs" \
...
echo "Dump convolution activations on the testing data"
mkdir -p $working_dir/data_conv
if [ ! -d $working_dir/data_conv/test ]; then
  cp -r $working_dir/data/test $working_dir/data_conv/test
  ( cd $working_dir/data_conv/test; rm -rf {cmvn,feats}.scp split*; )
fi
...
if [ ! -f $working_dir/txt.feat.done ]; then
  ...
  steps_pdnn/generate_txt_fbank.sh --cmd "$train_cmd"  \
    ...
    $working_dir/data/test $working_dir/_log $working_dir || exit 1;
  if [ ! -f $working_dir/fbank_txt_test.ark ]; then
    echo "No fbank_txt_*.ark was generated" && exit 1;
  fi
...
if [ ! -f $working_dir/conv.feat.done ]; then
  ...
  $cmd $working_dir/_log/conv.feat.log \
    export PYTHONPATH=$PYTHONPATH:`pwd`/pdnn/ \; \
    export THEANO_FLAGS=mode=FAST_RUN,device=$gpu,floatX=float32 \; \
    $pythonCMD pdnn/run_CnnFeat.py --ark-file $working_dir/fbank_txt_test.ark \
                          --conv-net-file $working_dir/conv.nnet --conv-layer-number 2 \
                          --wdir $working_dir --output-file-prefix $working_dir/_conv/conv_test || exit 1;
  cp $working_dir/_conv/conv_test.scp $working_dir/data_conv/test/feats.scp

  # It's critical to generate "fake" CMVN states here.
  steps/compute_cmvn_stats.sh --fake \
      $working_dir/data_conv/test $working_dir/_log $working_dir/_conv || exit 1;  
...
if [ ! -f  $working_dir/decode.done ]; then
  ...
  # Decode test
  steps_pdnn/decode_dnn.sh --nj 20 --beam 22 --latbeam 12 --scoring-opts "--min-lmwt 1 --max-lmwt 8" \
    --cmd "$decode_cmd" --norm-vars false \
    $graph_dir $working_dir/data_conv/test ${gmmdir}_ali $working_dir/decode || exit 1;
