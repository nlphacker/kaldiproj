...
echo ---------------------------------------------------------------------
echo "Creating SAT-DNN training and validation pfiles"
echo ---------------------------------------------------------------------
# By default, DNN inputs include 11 frames (+/-5) of 40-dimensional log-scale filter-banks, 
...
echo ---------------------------------------------------------------------
echo "Starting SAT-DNN training"
echo ---------------------------------------------------------------------
...
if [ ! -f  $working_dir/decode.done ]; then
  ...
  steps_pdnn/decode_dnn_ivec.sh --nj 24 --scoring-opts "--min-lmwt 8 --max-lmwt 16" --cmd "$decode_cmd" \
    --norm-vars true --is-spk-mode true \
