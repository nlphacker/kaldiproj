...
for set in dev test; do
  ...
  if [ ! -f $working_dir/conv.feat.$set.done ]; then
    ...
    # Since we used the fast version when calling run_CNN.py, now it has to be the fast version.
...
if [ ! -f  $working_dir/decode.done ]; then
  ...
  # Decode
  steps_pdnn/decode_dnn.sh --nj 8 --scoring-opts "--min-lmwt 1 --max-lmwt 8" --cmd "$decode_cmd" --norm-vars false \
