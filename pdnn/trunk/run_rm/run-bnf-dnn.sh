...
# The scripts up to now are the same as run-bnf-tandem.sh. You may want to reuse things from there.
...
if [ ! -f $working_dir/train.pfile2.done ]; then
  steps_pdnn/build_nnet_pfile.sh --cmd "$train_cmd" --every-nth-frame 1 --norm-vars false \
    ...
    $datadir/train ${gmmdir}_ali $working_dir || exit 1
  ( cd $working_dir; rm concat.pfile; )
...
if [ ! -f  $working_dir/decode.done ]; then
  ...
  steps_pdnn/decode_dnn.sh --nj 20 --beam 22 --latbeam 12 --scoring-opts "--min-lmwt 2 --max-lmwt 10" \
    ...
    $graph_dir $datadir/test ${gmmdir}_ali $working_dir/decode || exit 1;
